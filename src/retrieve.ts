...
export class RetrieveDialog {
    ...
    private async generatePyJSON(connectionId) : Promise<void> {
        ...
        let providerCustomCSQueryCommand : string = `SELECT [connection_name], [custom_connect_string], [provider] FROM [elt].[oledb_connection]`;
        ...
        try {
            ...
            var connection_names = resultCustConn.rows.map(cuci=> cuci[0]['displayValue']).filter((v, i, a) => a.indexOf(v) === i);   
            let curent_con_string = resultCustConn.rows.map(cuci=> cuci[1]['displayValue']).filter((v, i, a) => a.indexOf(v) === i && v!=="");
            var curent_con_string_list = curent_con_string.join("','");
            var provider_list = resultCustConn.rows.map(cuci=> cuci[2]['displayValue']).filter((v, i, a) => a.indexOf(v) === i);
        ...
        try {
            ...
            let old_connection_name = `$`+ `{1|${connection_names}|}`;
        ...
        try {
            ...
            let batch_size =  `$`+ `{11:batch size default(0)}`;
        ...
        try {
            ...
            let foreach_connection = `$`+ `{1|${connection_names}|}`;
            let connection_name_source = `$`+ `{2|${connection_names}, '@[User::Item]|}`;
            let foreach_query = `$`+ `{3: \* foreach query */ }`;    
            let query = `$`+ `{4: \* source query */ }`;
            let package_qualifier =  `$`+ `{5:package_qualifier}`;
            let destination_connection = `$`+ `{6|${connection_names}|}`;
        ...
        try {
            ...
            let working_directory =  `$`+ `{4|default, ${parameter_references}|}`;
    ...
    private async getQuery(engineType: string): Promise < void > {
        ...
        if (engineType === "eltSnap") {
            if(this.sourceQueryRadioBtn.checked && this.sourceQueryRadioBtn.enabled){
                ...
                const connection = ADODB.open(forEachConnString);

                try {
                    let forEachResult = await connection.query(forEachQuery);
